...
export function readCapabilityState(status, options = {}) {
  ...
  let modeReason = null
  if (!sessionReady) modeReason = 'Load a model and wait for fresh session status.'
  else if (blocked) modeReason = transitionPending
    ? 'Refreshing session status.' : 'Another operation is in progress.'
  const modes = Object.fromEntries(MODE_IDS.map((id) => {
    ...
    return [id, { decision, enabled: sessionReady && !blocked,
      mechanicalReason: modeReason, selected: id === selectedMode }]
  }))
  ...
  const formats = Object.fromEntries(EXPORT_IDS.map((id) => {
    ...
    if (blocked) reasons.push(transitionPending ? 'Refreshing session status.' : 'Another operation is in progress.')
    const local = { ready: reasons.length === 0, reason: reasons.join(' ') || null }
    return [id, { decision, local, enabled: sessionReady && local.ready }]
  }))
